...
def finance_add(date: str, expense: int) -> str:
    ...
    return '<a href="/finance">back</a>' \
           '<h2>Добавление прошло успешно! Можно проверять дальше)</h2>'


@server.route('/finance/calculate/<int:year>')
def finance_year(year: int) -> str:
    return '<a href="/finance">back</a><br>' \
           f'Расход в {year} составил {sum(storage.get(year).values())}' if year in storage.keys() else \
           f'В {year} расходов не было'
...
def finance_year_and_month(date: str) -> str:
    ...
    if month not in storage.get(year).keys():
        return '<a href="/finance">back</a><br>' \
               f'В {months[month-1]} {year} расходов не было'
    return '<a href="/finance">back</a><br>' \
           f'Расход в {months[month-1]} {year} составил {storage.get(year).get(month)}'
